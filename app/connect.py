...
class ConnectTask:
    ...
    def __init__(self, mq_client, kafka_client, config):
        self.stop = False
    ...
    def mq_read_callback(self, channel, basic_delivery, properties, body):
        def message_callback(err, msg):
            if err is None:
                ...
                print(f'Ack messages to the MQ client {basic_delivery.delivery_tag}')
        ...
        ## produce into kafka
        self.kafka_client.produce(topic=self.topic, message=body, message_callback=message_callback)
